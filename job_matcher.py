...
#         query = """
#         SELECT id, company, job_title, company, 
...
class JobFinder:
    ...
    def _load_jobs_data(self):
        """Load jobs data from SQLite database"""
        query = """
        SELECT id, job_title, company, location, description, requirements, company_email
        FROM jobs
        """
        try:
            df = pd.read_sql(query, self.conn)
...
def main():
    current_dir = Path(__file__).parent
    pdf_path = current_dir / "resume.pdf"
    db_path = current_dir / "main.db"  # Updated to use main.db
    api_key = "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"
